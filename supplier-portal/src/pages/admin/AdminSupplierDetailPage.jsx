...
import {
  ...
  CreditCard, FileText, User, Edit3, Save, X,
} from 'lucide-react';
...
const AdminSupplierDetailPage = () => {
  ...
  const load = () => {
    ...
    adminService.getSupplier(id)
      .then((res) => { setProfile(res.data); setEditForm({ full_name: res.data?.full_name || '', email: res.data?.email || '', phone: res.data?.phone || '' }); })
  ...
  const handleDelete = async () => {
    ...
    try { await adminService.deleteSupplier(id); navigate('/admin/suppliers'); } catch {} finally { setActionLoading(false); setShowDelete(false); }
  };

  const handleEditSave = async () => {
    setActionLoading(true);
    try { const res = await adminService.updateSupplier(id, editForm); setProfile(res.data); setEditing(false); } catch {} finally { setActionLoading(false); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Section title="Category & Brand" icon={FileText}>
        ...
        <Section title="Documents" icon={FileText}>
          <div className="space-y-2">
            {DOC_LIST.map((doc) => {
              ...
              return (
                <div key={doc.key} className={`flex items-center justify-between px-3 py-2 rounded-lg border ${uploaded ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    {d?.document_url && (
                      <a href={`http://localhost:8000${d.document_url}`} target="_blank" rel="noreferrer"
                        className="text-[10px] text-violet-600 font-semibold hover:underline ml-1">View</a>
                    )}
                  </div>
                </div>
              );
