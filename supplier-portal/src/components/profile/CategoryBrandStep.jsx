  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-surface rounded-2xl border border-border p-6">
        <h2 className="text-lg font-bold text-highlight mb-1">Category Selection</h2>
        <p className="text-xs text-muted mb-4">Select the categories you sell in</p>
        ...
        <div className="flex gap-2">
          <select value={catInput} onChange={(e) => { addCategory(e.target.value); e.target.value = ''; }} className="flex-1 px-4 py-2.5 border border-border rounded-xl text-sm outline-none focus:ring-1 focus:ring-primary bg-[#0A0D14] text-highlight">
      ...
      {/* Brands */}
      <div className="bg-surface rounded-2xl border border-border p-6">
        <h2 className="text-sm font-bold text-highlight mb-1">Brands</h2>
        <p className="text-[10px] text-muted mb-4">Brands Require Authorized Letter</p>
        <div className="flex flex-wrap gap-2 mb-3">
          {brands.map((b) => (
        ...
        <div className="flex gap-2">
          <input value={brandInput} onChange={(e) => setBrandInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (addBrand(brandInput), e.preventDefault())} className="flex-1 px-4 py-2.5 border border-border rounded-xl text-sm outline-none focus:ring-1 focus:ring-primary bg-[#0A0D14] text-highlight placeholder-muted" placeholder="Type brand name…" />
      ...
      <div className="bg-surface rounded-2xl border border-border p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-highlight">Brand Details</h3>
              ...
              <tbody>
                {brandRows.map((r, i) => (
                  <tr key={r.id} className="border-b border-border">
                    <td className="px-3 py-2"><input value={r.name} onChange={(e) => updateRow(i, 'name', e.target.value)} className="px-2 py-1 border border-border rounded-lg text-xs w-full bg-[#0A0D14] text-highlight" placeholder="Brand" /></td>
                    <td className="px-3 py-2"><select value={r.nature} onChange={(e) => updateRow(i, 'nature', e.target.value)} className="px-2 py-1 border border-border rounded-lg text-xs bg-[#0A0D14] text-highlight"><option value="">Select</option>{NATURES.map((n) => <option key={n}>{n}</option>)}</select></td>
                    <td className="px-3 py-2"><FileUpload value={r.cert} onChange={(f) => updateRow(i, 'cert', f)} /></td>
                    <td className="px-3 py-2"><input type="date" value={r.date} onChange={(e) => updateRow(i, 'date', e.target.value)} className="px-2 py-1 border border-border rounded-lg text-xs bg-[#0A0D14] text-highlight" /></td>
