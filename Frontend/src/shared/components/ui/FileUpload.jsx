...
function FileUpload({ label, accept, maxSize = 10, value, onChange, status = 'Not Uploaded', disabled = false }) {
  ...
  const openPicker = (e) => { e.stopPropagation(); if (disabled) return; inputRef.current?.click(); };
  const clearFile = (e) => { e.stopPropagation(); onChange(null); if (inputRef.current) inputRef.current.value = ''; };
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input ref={inputRef} type="file" className="hidden" accept={accept} onChange={(e) => { handleFile(e.target.files); e.target.value = ''; }} />
      <div
        onClick={openPicker}
        onDragOver={(e) => { e.preventDefault(); setDragging(true); }}
        onDragLeave={() => setDragging(false)}
        onDrop={(e) => { e.preventDefault(); setDragging(false); handleFile(e.dataTransfer.files); }}
        className={cn('relative border-2 rounded-xl p-4 text-center transition-all cursor-pointer', statusColor, dragging && 'border-violet-500 bg-violet-50', disabled && 'opacity-50 cursor-not-allowed')}
      >
        {hasFile ? (
