...
const AdminProductsPage = () => {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-xl font-bold text-gray-800">Products</h1><p className="text-xs text-gray-500 mt-0.5">Manage all supplier products</p></div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        : products.length === 0 ? <div className="text-center py-20"><Package size={40} className="mx-auto text-gray-300 mb-3" /><p className="text-sm font-semibold text-gray-500">No products found</p></div>
        : (
          <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>{products.map((p) => (
                  <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3"><div className="flex items-center gap-1.5"><Link to={`/admin/products/${p.id}`} className="p-1.5 text-gray-400 hover:text-violet-600 cursor-pointer" title="View"><Eye size={14} /></Link>{p.status === 'pending' && <button onClick={() => handleQuickApprove(p.id)} className="p-1.5 text-gray-400 hover:text-green-600 cursor-pointer" title="Approve"><CheckCircle size={14} /></button>}</div></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
            <div className="lg:hidden space-y-3 p-4">{products.map((p) => (
              <div key={p.id} className="border border-gray-200 rounded-xl p-4 space-y-2">
                ...
                <div className="flex items-center justify-between text-xs"><span className="text-gray-500">Price: <span className="font-bold text-gray-800">{formatPrice(p.price)}</span></span><span className="text-gray-500">Stock: <span className="font-semibold">{p.stock}</span></span>{p.status === 'pending' && <button onClick={() => handleQuickApprove(p.id)} className="text-green-600 font-bold cursor-pointer">Approve</button>}</div>
              </div>
            ))}</div>
          </>
        )}
