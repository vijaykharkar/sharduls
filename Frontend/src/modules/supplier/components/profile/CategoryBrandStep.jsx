    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {categories.map((c) => (
            <span key={c} className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-50 text-violet-700 text-xs font-semibold rounded-full border border-violet-200">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {brands.map((b) => (
            <span key={b} className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-50 text-violet-600 text-xs font-semibold rounded-full border border-violet-200">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => addBrand(brandInput)} className="px-4 py-2.5 bg-violet-600 text-white text-xs font-bold rounded-xl cursor-pointer hover:bg-violet-700"><Plus size={14} /></button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={addBrandRow} className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-600 text-white text-xs font-bold rounded-lg cursor-pointer hover:bg-violet-700"><Plus size={14} /> Add New Brand</button>
        </div>
        {brandRows.length === 0 ? (
          <div className="text-center py-10 text-gray-400"><p className="text-3xl mb-2">🏷️</p><p className="text-xs">No brands added yet</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-gray-50">
                <th className="text-left px-3 py-2 text-[10px] font-bold text-gray-500 uppercase">Brand Name</th>
                <th className="text-left px-3 py-2 text-[10px] font-bold text-gray-500 uppercase">Nature</th>
                <th className="text-left px-3 py-2 text-[10px] font-bold text-gray-500 uppercase">Certificate</th>
                <th className="text-left px-3 py-2 text-[10px] font-bold text-gray-500 uppercase">Date</th>
                <th className="text-left px-3 py-2 text-[10px] font-bold text-gray-500 uppercase">Status</th>
                <th className="text-left px-3 py-2 text-[10px] font-bold text-gray-500 uppercase">Action</th>
              </tr></thead>
              <tbody>
                {brandRows.map((r, i) => (
