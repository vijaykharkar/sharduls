    ...
    <div className="space-y-6">
          ...
          {DOC_LIST.map((doc) => {
            ...
            return (
              <motion.div
                key={doc.key}
                whileHover={{ y: -2 }}
                className={`rounded-xl border-2 p-4 transition-colors ${file ? 'border-violet-300 bg-violet-50/30' : 'border-dashed border-gray-200'}`}
              >
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          ...
          <p className="text-xs text-gray-500">Required: {uploadedRequired}/{requiredDocs.length}</p>
        </div>
        <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
          <motion.div className="h-full bg-violet-500 rounded-full" initial={{ width: 0 }} animate={{ width: `${(totalUploaded / DOC_LIST.length) * 100}%` }} transition={{ duration: 0.5 }} />
        </div>
      </div>

      {submitted && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-green-50 border border-green-200 rounded-2xl p-6 text-center">
          <div className="text-5xl mb-3">✨</div>
          <h3 className="text-lg font-bold text-gray-800 mb-1">Profile Submitted for Review!</h3>
          <p className="text-sm text-gray-500">We'll notify you within 2-3 business days.</p>
        </motion.div>
      )}
      ...
      <div className="flex justify-between">
        ...
        <button onClick={handleSubmit} disabled={!allRequiredUploaded || submitted} className="px-6 py-2.5 bg-violet-600 text-white font-bold rounded-xl hover:bg-violet-700 transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed">
