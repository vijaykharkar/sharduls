...
import { validateGSTIN, lookupPincode, validatePincode } from '@shared/utils/helpers';
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await profileService.saveBusinessDetails({
        legal_name: form.legalName, trade_name: form.tradeName, gstin: form.gstin,
        country: form.country, pincode: form.pincode, state: form.state, city: form.city,
        tan: form.tan || null, entity_type: form.entityType,
        has_udyam: form.hasUdyam === 'yes', udyam_file_url: null,
      });
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div className="flex gap-4 mt-1">
            <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="udyam" checked={form.hasUdyam === 'no'} onChange={() => up('hasUdyam', 'no')} className="accent-violet-600" /><span className="text-sm">No</span></label>
            <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="udyam" checked={form.hasUdyam === 'yes'} onChange={() => up('hasUdyam', 'yes')} className="accent-violet-600" /><span className="text-sm">Yes</span></label>
          </div>
        </div>
      </div>
