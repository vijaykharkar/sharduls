  ...
  const handleSave = async () => {
    ...
    try {
      const res = await profileService.saveContactDetails({
        primary: {
          contact_name: form.name, phone: form.phone, email: form.email,
          alt_email: form.altEmail || null, pickup_time: form.pickupTime,
        },
        others: others.map((o) => ({
          contact_name: o.name, phone: o.phone, email: o.email, location: o.location || null,
        })),
      });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setNewRow({ name: '', phone: '', email: '', location: '' })} className="flex items-center gap-1.5 px-3 py-1.5 bg-violet-600 text-white text-xs font-bold rounded-lg cursor-pointer hover:bg-violet-700 transition-colors">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="bg-gray-50">
                <th className="text-left px-4 py-2 text-[10px] font-bold text-gray-500 uppercase">Name</th>
                <th className="text-left px-4 py-2 text-[10px] font-bold text-gray-500 uppercase">Phone</th>
                <th className="text-left px-4 py-2 text-[10px] font-bold text-gray-500 uppercase">Email</th>
                <th className="text-left px-4 py-2 text-[10px] font-bold text-gray-500 uppercase">Location</th>
                <th className="text-left px-4 py-2 text-[10px] font-bold text-gray-500 uppercase">Action</th>
              </tr></thead>
              <tbody>
                {others.map((c, i) => (
                  <tr key={c.id || i} className="border-b border-gray-100 text-gray-500">
                ...
                {newRow && (
                  <tr className="border-b border-gray-100 bg-violet-50/30">
                    ...
                    <td className="px-2 py-2 flex gap-1">
                      ...
                      <button onClick={() => setNewRow(null)} className="px-2 py-1 bg-gray-200 text-gray-600 text-[10px] rounded-lg cursor-pointer">Cancel</button>
                    </td>
                  </tr>
                )}
              </tbody>
