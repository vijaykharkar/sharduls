...
const navItems = [
  ...
  { to: '/supplier/profile', icon: UserCircle, label: 'My Profile', showBadge: true },
  { to: '/supplier/orders', icon: ShoppingBag, label: 'Orders' },
  { to: '/supplier/products', icon: Package, label: 'Products' },
  { to: '/supplier/payments', icon: CreditCard, label: 'Payments' },
  { to: '/supplier/support', icon: Headphones, label: 'Support' },
];
    ...
    <div className="h-full flex flex-col sidebar-gradient rounded-r-3xl overflow-hidden text-white">
      <div className="p-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-sm tracking-wide text-highlight">SHARDUL-GE</p>
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-9 h-9 rounded-full border-2 border-primary/40 flex items-center justify-center text-primary font-bold text-sm flex-shrink-0">
            {user?.name?.[0] || user?.full_name?.[0] || 'S'}
          </div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-highlight truncate">{user?.full_name || user?.name || 'Supplier'}</p>
