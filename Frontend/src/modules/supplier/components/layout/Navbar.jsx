      ...
      if (isApproved) {
        ...
        try {
          ...
          if (review) {
            ...
            if (rejectedCount > 0) {
              notifs.push({ id: 'review', type: 'error', title: 'Admin Review Feedback', desc: `${rejectedCount} item${rejectedCount !== 1 ? 's' : ''} need attention. Check your dashboard for details.`, link: '/supplier/dashboard' });
            }
          }
        } catch { /* no review yet */ }
      ...
      <div className="flex items-center gap-3">
                ...
                <motion.div initial={{ opacity: 0, y: -8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.97 }}
                  ...
                  <div className="max-h-72 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n) => (
                        <Link key={n.id} to={n.link || '/supplier/dashboard'} onClick={() => setBellOpen(false)}
        ...
        <div className="relative">
          <button onClick={() => { setDropOpen(!dropOpen); setBellOpen(false); }} className="flex items-center gap-2 cursor-pointer hover:bg-white/5 rounded-xl px-2 py-1.5 transition-colors">
            <div className="w-8 h-8 rounded-full border-2 border-primary/40 flex items-center justify-center text-primary font-bold text-xs">{user?.name?.[0] || user?.full_name?.[0] || 'S'}</div>
            <span className="hidden sm:block text-sm font-medium text-highlight">{(user?.full_name || user?.name || 'Supplier').split(' ')[0]}</span>
          ...
          {dropOpen && (
            <>
              ...
              <div className="absolute right-0 top-12 w-48 bg-surface border border-border rounded-xl shadow-strong z-50 py-1 animate-slideDown">
                <button onClick={() => { setDropOpen(false); navigate('/supplier/profile'); }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-highlight hover:bg-white/5 transition-colors cursor-pointer">
                  <User size={16} /> Profile
                </button>
                <button onClick={async () => { setDropOpen(false); await logout(); navigate('/login'); }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-400 hover:bg-red-500/10 transition-colors cursor-pointer">
