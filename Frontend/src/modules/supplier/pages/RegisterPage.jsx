          ...
          <AnimatePresence mode="wait">
            {step === 1 ? (
              <motion.div key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.22 }} className="space-y-4">
                <div>
                  ...
                  {!otpSent && (
                    <button type="button" onClick={handleSendOtp} disabled={loading}
                      ...
                      {loading ? <Loader2 size={12} className="animate-spin" /> : null} Send OTP
                    </button>
                  )}
                  {otpSent && (
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">OTP <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">Email ID <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={terms} onChange={(e) => setTerms(e.target.checked)} className="w-4 h-4 rounded accent-violet-600" />
                    <span className="text-xs text-gray-600">I Accept The <span className="text-violet-600 font-semibold underline cursor-pointer">Terms and Conditions</span></span>
                  </label>
                  <FieldError name="terms" />
                </div>

                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={keepSigned} onChange={(e) => setKeepSigned(e.target.checked)} className="w-4 h-4 rounded accent-violet-600" />
                  <span className="text-xs text-gray-600">Keep me signed in <span className="text-violet-600 font-semibold underline cursor-pointer">Details</span></span>
                </label>
              ...
              <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.22 }} className="space-y-5">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">What is your business model? <span className="text-red-500">*</span></label>
                  <SingleSelect options={BUSINESS_MODELS} value={step2.businessModel} onChange={(v) => up2('businessModel', v)} placeholder="Select" error={errors.businessModel} />
                  <FieldError name="businessModel" />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">Products You Wish To Sell <span className="text-red-500">*</span></label>
                  <MultiSelect options={PRODUCT_CATEGORIES} value={step2.products} onChange={(v) => up2('products', v)} placeholder="Categories" error={errors.products} />
                  <FieldError name="products" />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">GSTIN <span className="text-red-500">*</span></label>
                  <input type="text" value={step2.gstin} onChange={(e) => up2('gstin', e.target.value.toUpperCase())} className={`${ic} uppercase tracking-widest`} placeholder="Enter GSTIN" maxLength={15} />
                  <FieldError name="gstin" />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1.5">Create Password <span className="text-red-500">*</span></label>
                  <div className="relative">
                    <input type={showPwd ? 'text' : 'password'} value={step2.password} onChange={(e) => up2('password', e.target.value)} className={ic + ' pr-10'} placeholder="Min 8 chars, 1 uppercase, 1 digit" />
                    <button type="button" onClick={() => setShowPwd((p) => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-violet-600">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showConfirmPwd ? 'text' : 'password'} value={step2.confirmPassword} onChange={(e) => up2('confirmPassword', e.target.value)} className={ic + ' pr-10'} placeholder="Re-enter your password" />
                    <button type="button" onClick={() => setShowConfirmPwd((p) => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-violet-600">
