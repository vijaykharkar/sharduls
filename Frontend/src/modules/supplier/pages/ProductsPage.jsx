...
const ProductsPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        description: editProduct.description || '', brand: editProduct.brand || '',
        sku: editProduct.sku || '', price: parseFloat(editProduct.price),
        mrp: editProduct.mrp ? parseFloat(editProduct.mrp) : null,
        stock: parseInt(editProduct.stock || 0, 10), images: allImages,
      };
      if (editProduct.id) {
  ...
  const ic = 'w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm outline-none focus:ring-1 focus:ring-violet-300 bg-white text-gray-800 placeholder-gray-400 transition-all';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <button onClick={openAdd} className="mt-4 px-4 py-2 bg-violet-600 text-white text-xs font-bold rounded-xl cursor-pointer">Add First Product</button>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Product</th>
                    <th className="text-left px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Category</th>
                    <th className="text-left px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Price</th>
                    <th className="text-left px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Stock</th>
                    <th className="text-left px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Status</th>
                    <th className="text-left px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {products.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-5 py-3 font-semibold text-gray-800">{formatPrice(p.supplier_price ?? p.price)}</td>
                        ...
                        <td className="px-5 py-3">
                          <div className="flex gap-2">
                            <button onClick={() => openEdit(p)} className="text-gray-400 hover:text-violet-600 cursor-pointer transition-colors"><Edit2 size={15} /></button>
                            <button onClick={() => setDeleteId(p.id)} className="text-gray-400 hover:text-red-400 cursor-pointer transition-colors"><Trash2 size={15} /></button>
                          </div>
                        </td>
                      </tr>
                    );
                ...
                return (
                  <div key={p.id} className="border border-gray-200 rounded-xl p-4 bg-gray-50">
                    <div className="flex items-center gap-3 mb-2">
                      <img src={img} alt="" className="w-12 h-12 rounded-lg object-cover border border-gray-200" />
                    ...
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-gray-800">{formatPrice(p.supplier_price ?? p.price)}</span>
                      <div className="flex gap-2">
                        <button onClick={() => openEdit(p)} className="text-gray-400 hover:text-violet-600 cursor-pointer"><Edit2 size={15} /></button>
                        <button onClick={() => setDeleteId(p.id)} className="text-gray-400 hover:text-red-400 cursor-pointer"><Trash2 size={15} /></button>
                      </div>
                    </div>
                  </div>
                );
      ...
      <ConfirmModal isOpen={deleteId !== null} onClose={() => setDeleteId(null)} onConfirm={handleDelete}
        title="Delete Product" message="This product will be permanently deleted and cannot be recovered." confirmText="Delete" danger />
    </div>
  );
};
