...
const ProfilePage = () => {
  ...
  const goToStep = (i) => { if (profileSteps[STEPS[i].key] || i <= currentStep) setCurrentStep(i); };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6">
      <div className="lg:w-64 flex-shrink-0">
        <div className="lg:sticky lg:top-20 bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
          <div className="text-center mb-4">
            <p className="text-2xl font-bold text-gray-800">{completionPercentage}%</p>
            <p className="text-[10px] text-gray-500 uppercase tracking-wider">Profile Complete</p>
            <div className="w-full h-1.5 bg-gray-100 rounded-full mt-2 overflow-hidden">
              <motion.div className="h-full bg-violet-500 rounded-full" initial={{ width: 0 }} animate={{ width: `${completionPercentage}%` }} transition={{ duration: 0.8 }} />
            </div>
          </div>
          ...
          <div className="hidden lg:block space-y-1">
            {STEPS.map((step, i) => {
              ...
              return (
                <button key={step.key} onClick={() => goToStep(i)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all cursor-pointer ${
                    active ? 'bg-violet-50 border border-violet-200' : 'hover:bg-gray-50'
                  }`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold transition-all ${
                    done ? 'bg-violet-600 text-white' : active ? 'bg-violet-100 text-violet-700' : 'bg-gray-100 text-gray-400'
                  }`}>
                    {done ? <Check size={14} /> : i + 1}
                  </div>
                  <div className="min-w-0">
                    <p className={`text-xs font-semibold truncate ${active ? 'text-violet-700' : 'text-gray-800'}`}>{step.label}</p>
                    <p className="text-[9px] text-gray-400">{done ? 'Complete ✓' : active ? 'In Progress' : 'Pending'}</p>
                  </div>
                  {active && <ChevronRight size={14} className="ml-auto text-violet-600" />}
                </button>
              );
            })}
          </div>
