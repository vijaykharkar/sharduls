...
export default function CartPage() {
  ...
  const { isAuthenticated, openAuthModal } = useBuyerAuth();
  const { role } = useAuth();
  const isBuyer = isAuthenticated && (role || '').toLowerCase() === 'buyer';
      ...
      <div className="flex flex-col lg:flex-row gap-6">
          ...
          <AnimatePresence>
            {cartItems.map((item) => (
              <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -100 }}
                ...
                <Link to={`/buyer/product/${item.id}`} className="flex-shrink-0">
                  <img src={item.image} alt={item.name} className="w-20 h-20 sm:w-24 sm:h-24 object-cover rounded-xl" />
                </Link>
                <div className="flex-1 min-w-0">
                  <Link to={`/buyer/product/${item.id}`} className="text-sm font-semibold text-gray-800 line-clamp-2 hover:text-[#d4a853] transition-colors">{item.name}</Link>
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    ...
                    <button onClick={() => removeFromCart(item.id)} className="text-red-400 hover:text-red-600 transition-colors cursor-pointer"><Trash2 size={15} /></button>
                  </div>
                </div>
        ...
        <div className="lg:w-80 flex-shrink-0">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-card p-5 sticky top-20">
            ...
            {discount > 0 && <p className="text-xs text-green-600 font-semibold mt-3 bg-green-50 rounded-lg px-3 py-2 text-center">You save ₹{fmt(discount)} on this order</p>}
            {isBuyer ? (
              <Link to="/buyer/checkout" className="block w-full mt-4 py-3 bg-[#0a1929] text-white rounded-xl text-sm font-bold text-center hover:bg-[#102a43] transition-colors">Proceed to Checkout</Link>
            ) : (
              <button onClick={() => openAuthModal('login')} className="w-full mt-4 py-3 bg-[#0a1929] text-white rounded-xl text-sm font-bold hover:bg-[#102a43] transition-colors cursor-pointer">Login to Checkout</button>
            )}
          </div>
        </div>
      </div>
