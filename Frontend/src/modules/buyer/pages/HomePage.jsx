    ...
    <div className="space-y-8 sm:space-y-10">
        ...
        <div className="relative px-5 sm:px-10 py-10 sm:py-16 text-white max-w-2xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-6">
              <Link to="/buyer/products" className="inline-flex items-center justify-center gap-2 bg-[#d4a853] text-[#0a1929] px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-[#e0c07a] transition-colors">
                Shop Now <ArrowRight size={16} />
              </Link>
              <Link to="/buyer/products?category=Electronics" className="inline-flex items-center justify-center gap-2 bg-white/10 backdrop-blur text-white border border-white/20 px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-white/20 transition-colors">
      ...
      <section className="px-4 sm:px-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/buyer/products" className="text-xs text-[#d4a853] font-semibold hover:underline flex items-center gap-1">View All <ArrowRight size={12} /></Link>
        </div>
        <div className="flex gap-2 sm:gap-3 overflow-x-auto scrollbar-hidden pb-2">
          {CATEGORIES.filter((c) => c !== 'All').map((cat) => (
            <Link key={cat} to={`/buyer/products?category=${cat}`}
      ...
      <section className="px-4 sm:px-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/buyer/products" className="text-xs text-[#d4a853] font-semibold hover:underline flex items-center gap-1">See All <ArrowRight size={12} /></Link>
        </div>
        {loading ? (
      ...
      <section className="px-4 sm:px-6">
        <div className="bg-gradient-to-br from-[#0a1929] to-[#102a43] rounded-2xl p-5 sm:p-8 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Link to="/buyer/products?category=Electronics" className="inline-flex items-center gap-2 bg-[#d4a853] text-[#0a1929] px-5 sm:px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-[#e0c07a] transition-colors flex-shrink-0">
