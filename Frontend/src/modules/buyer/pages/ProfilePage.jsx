...
export default function ProfilePage() {
  ...
  const quickLinks = [
    { to: '/buyer/orders', icon: Package, label: 'My Orders', desc: 'Track, return, cancel orders' },
    { to: '/buyer/wishlist', icon: Heart, label: 'Wishlist', desc: `${wishlistCount} saved item${wishlistCount !== 1 ? 's' : ''}` },
    { to: '/buyer/cart', icon: Package, label: 'Cart', desc: `${cartCount} item${cartCount !== 1 ? 's' : ''} in cart` },
    { to: '/buyer/checkout', icon: MapPin, label: 'Addresses', desc: `${addresses.length} saved address${addresses.length !== 1 ? 'es' : ''}` },
  ];
