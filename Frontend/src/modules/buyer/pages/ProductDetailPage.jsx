...
export default function ProductDetailPage() {
  ...
  if (!product) return (
    <div className="text-center py-20 max-w-7xl mx-auto px-4">
      ...
      <Link to="/buyer/products" className="mt-4 inline-block px-5 py-2 bg-[#0a1929] text-white rounded-xl text-sm font-semibold">Browse Products</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-10">
      <nav className="flex items-center gap-1.5 text-xs text-gray-500">
        <Link to="/buyer" className="hover:text-[#d4a853]">Home</Link>
        <ChevronRight size={12} />
        <Link to="/buyer/products" className="hover:text-[#d4a853]">Products</Link>
        <ChevronRight size={12} />
        <Link to={`/buyer/products?category=${product.category}`} className="hover:text-[#d4a853]">{product.category}</Link>
        <ChevronRight size={12} />
