...
export default function BuyerNavbar() {
  const { isAuthenticated, user, logout, openAuthModal } = useBuyerAuth();
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (search.trim()) {
      navigate(`/buyer/products?search=${encodeURIComponent(search.trim())}`);
      setSearch('');
      setMobileMenu(false);
    }
  };
    ...
    <header className="sticky top-0 z-50 bg-[#0a1929] shadow-md">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
        <div className="flex items-center h-14 sm:h-16 gap-2 sm:gap-4">
          <Link to="/buyer" className="flex items-center gap-2 flex-shrink-0">
            {logo ? <img src={logo} alt="Shardul-GE" className="h-8 sm:h-9 w-auto object-contain" /> : <span className="text-white font-bold text-lg">Shardul-GE</span>}
          </Link>
          ...
          <div className="flex items-center gap-0.5 sm:gap-1.5 ml-auto">
            <Link to="/buyer/wishlist" className="relative p-2 text-white/80 hover:text-[#d4a853] transition-colors">
              <Heart size={20} />
              {wishlistCount > 0 && <span className="absolute -top-0.5 -right-0.5 min-w-[18px] px-1 h-[18px] bg-[#d4a853] text-[#0a1929] text-[9px] font-bold rounded-full flex items-center justify-center">{wishlistCount}</span>}
            </Link>
            <Link to="/buyer/cart" className="relative p-2 text-white/80 hover:text-[#d4a853] transition-colors">
              <ShoppingCart size={20} />
              {cartCount > 0 && <span className="absolute -top-0.5 -right-0.5 min-w-[18px] px-1 h-[18px] bg-[#d4a853] text-[#0a1929] text-[9px] font-bold rounded-full flex items-center justify-center">{cartCount}</span>}
            </Link>
                ...
                {profileDrop && (
                  <>
                    ...
                    <div className="absolute right-0 top-11 w-48 bg-white border border-gray-200 rounded-xl shadow-lg z-50 py-1 animate-slideDown">
                      <Link to="/buyer/profile" onClick={() => setProfileDrop(false)} className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#fdf8ed] transition-colors">
                        <User size={16} /> My Profile
                      </Link>
                      <Link to="/buyer/orders" onClick={() => setProfileDrop(false)} className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#fdf8ed] transition-colors">
        ...
        {mobileMenu && (
          <div className="md:hidden pb-4 space-y-3 border-t border-white/10 pt-3 animate-slideDown">
            ...
            <div className="flex flex-col gap-2">
              <Link to="/buyer/wishlist" onClick={() => setMobileMenu(false)} className="flex items-center gap-2 px-3 py-2 text-sm text-white/80 hover:text-[#d4a853]">
                <Heart size={16} /> Wishlist {wishlistCount > 0 && `(${wishlistCount})`}
              </Link>
              <Link to="/buyer/orders" onClick={() => setMobileMenu(false)} className="flex items-center gap-2 px-3 py-2 text-sm text-white/80 hover:text-[#d4a853]">
                <Package size={16} /> My Orders
              </Link>
            </div>
            {!isAuthenticated && (
